...
def part_one(data):
    known_values = 0
    for line in data:
        inputs, outputs = input_outputs(line)
        list_output = outputs.split(' ')
        kown_value = filter(lambda value: len(value) in [2, 3, 4, 7], list_output)
        if kown_value:
            known_values += len(kown_value)
    return known_values


def map_segments(value):
    segments = [
        'acedgfb','cdfbe','gcdfa','fbcad','dab','cefabd','cdfgeb','eafb','cagedb','ab'
    ]
    candidates = filter(lambda x: len(x) == len(value), segments)
    chars=[]
    chars[:0]=value
    ...
    for segment in candidates:
        ...
        for idx, char in enumerate(chars):
            if char in segment:
                segment_result.append(segment[idx])
        joined = ''.join(segment_result)
        if joined == segment:
            break
        else:
            joined = None
...
def part_two(data):
    ...
    for line in data:
        ...
        for value in list_output:
            joined = map_segments(value)
            if joined:
                total_output.append(str(dict_segments.get(joined)))
        # print(total_output)
        total_output = int(''.join(total_output)) if total_output else 0
        print(total_output)
        total += total_output
        # break
    return total
